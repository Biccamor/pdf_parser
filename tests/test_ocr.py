...
class TestGetTextOllama:
    ...
    @patch("ocr.chat")
    def test_returns_extracted_text(self, mock_chat, mock_image_file):
        """Poprawna odpowiedź modelu → zwraca tekst."""
        mock_response = MagicMock()
        mock_response.message.content = "Education\nWarsaw University 2020"
        mock_chat.return_value = mock_response

        result = get_text_ollama(mock_image_file)
        assert result == "Education\nWarsaw University 2020"

    @patch("ocr.chat")
    def test_strips_whitespace(self, mock_chat, mock_image_file):
    ...
    @patch("ocr.chat")
    def test_passes_image_bytes_to_chat(self, mock_chat, mock_image_file):
        """Sprawdza, że bajty obrazu są przekazywane do modelu."""
        mock_response = MagicMock()
        mock_response.message.content = "text"
        mock_chat.return_value = mock_response

        get_text_ollama(mock_image_file)
    ...
    @patch("ocr.chat")
    def test_uses_default_model(self, mock_chat, mock_image_file):
    ...
    @patch("ocr.chat")
    def test_custom_model(self, mock_chat, mock_image_file):
        """Przekazanie custom modelu."""
        mock_response = MagicMock()
        mock_response.message.content = "text"
        mock_chat.return_value = mock_response

        get_text_ollama(mock_image_file, model="custom-ocr:v2")
    ...
    @patch("ocr.chat")
    def test_response_error_raises_503(self, mock_chat, mock_image_file):
    ...
    @patch("ocr.chat")
    def test_connection_error_raises_503(self, mock_chat, mock_image_file):
    ...
    @patch("ocr.chat")
    def test_temperature_zero(self, mock_chat, mock_image_file):
